...
describe('File Operations Tools Integration', () => {
  ...
  test('Verify workspace is accessible via OpenTerminal', async () => {
    ...
    const { status, data } = await api(
      'GET',
      `/v1/sandbox/sessions/${sandboxSessionId}/files`
    );
  ...
  test('Workflow with file operations - Clone and analyze repo', async () => {
    ...
    const toolCalls = trace.filter(step => 
      step['step_type'] === 'subagent_tool_call' || step['step_type'] === 'tool_call'
    );
  ...
  test('Agent API with file_read tool', async () => {
    ...
    await api(
      'PUT',
      `/v1/sandbox/sessions/${sandboxSessionId}/file/test-config.json`,
  ...
  test('Agent API with grep tool', async () => {
    ...
    await api(
      'PUT',
      `/v1/sandbox/sessions/${sandboxSessionId}/file/file1.txt`,
      { content: 'Hello world\nThis is a test file\nFoo bar baz' }
    );
    
    await api(
      'PUT',
      `/v1/sandbox/sessions/${sandboxSessionId}/file/file2.txt`,
