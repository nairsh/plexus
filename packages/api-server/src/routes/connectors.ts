...
export async function connectorsRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post(
    ...
    async (request: FastifyRequest<{ Params: { provider: ConnectorProvider } }>) => {
  ...
  fastify.get(
    ...
    async (
      request: FastifyRequest<{
        Params: { provider: ConnectorProvider };
      ...
      if (request.query.error) {
        return reply
          .type('text/html')
          .send(getCallbackErrorHtml(request.query.error_description ?? request.query.error));
      }
      ...
      try {
        ...
        return reply
          .type('text/html')
          .send(getCallbackErrorHtml(error instanceof Error ? error.message : String(error)));
      }
    }
  );
  ...
  fastify.post('/v1/connectors/:id/proxy', async (request: FastifyRequest<{ Params: { id: string } }>) => {
  ...
  fastify.post(
    ...
    async (request: FastifyRequest<{ Params: { id: string; approvalId: string } }>) => {
      ...
      resolveWorkflowApproval(
        ...
        parsed.data.decision as ToolApprovalDecision
      );
